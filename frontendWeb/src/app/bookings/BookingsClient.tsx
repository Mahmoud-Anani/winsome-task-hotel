...
import { Calendar, User, DollarSign, Building2, DoorOpen } from "lucide-react";
...
export function BookingsClient({ initialBookings, user, t, locale }: BookingsClientProps) {
  ...
  const { data: allBookings = [] as any[], isLoading } = useQuery({
      ...
      <div className="grid grid-cols-3 md:grid-cols-4 gap-4 mb-8">
        ...
        <Card className="bg-yellow-500/5">
          <CardContent className="p-4 text-center">
            ...
            <p className="text-sm text-muted-foreground">{t["bookings.pending"]}</p>
          </CardContent>
        </Card>
        <Card className="bg-green-500/5">
          <CardContent className="p-4 text-center">
            <p className="text-2xl font-bold text-green-600">{confirmedCount}</p>
            <p className="text-sm text-muted-foreground">{t["bookings.confirmed"]}</p>
          </CardContent>
        </Card>
        <Card className="bg-red-500/5">
          <CardContent className="p-4 text-center">
            <p className="text-2xl font-bold text-red-600">{cancelledCount}</p>
            <p className="text-sm text-muted-foreground">{t["bookings.cancelled"]}</p>
          </CardContent>
        </Card>
      </div>
                ...
                <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
                  <div className="space-y-3 w-full lg:w-auto">
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      <Badge
                        variant={booking.status === "CONFIRMED" ? "success" : booking.status === "PENDING" ? "warning" : "destructive"}
                      >
                  ...
                  <div className="flex gap-2 w-full lg:w-auto justify-end">
                    {booking.status === "PENDING" && (user?.role === "ADMIN" || booking.user?.id === user?.id) && (
                      <Button size="sm" onClick={() => payMutation.mutate(booking.id)} disabled={payMutation.isPending}>
                        {t["bookings.pay"]}
                      </Button>
                    )}
                    {user?.role === "ADMIN" && (
                      <>
                        {booking.status === "PENDING" && (
                          <Button size="sm" onClick={() => updateStatusMutation.mutate({ id: booking.id, status: "CONFIRMED" })} disabled={updateStatusMutation.isPending}>
                            {t["bookings.confirm"]}
                          </Button>
                        )}
                        {(booking.status === "PENDING" || booking.status === "CONFIRMED") && (
                          <Button variant="destructive" size="sm" onClick={() => updateStatusMutation.mutate({ id: booking.id, status: "CANCELLED" })} disabled={updateStatusMutation.isPending}>
                            {t["bookings.cancel"]}
                          </Button>
                        )}
                      </>
